            ...
            <div class="card-body text-dark">
                <div class="form-row">
                    <div class="col">
                        ...
                        <div class="mb-3">
                            ...
                            <input class="form-control" id="quantity" type="number" name="price" value="{{ old('price', $product->price ?? '') }}">
                            @error('price')
                            <span class="font-italic text-danger ">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="col">
                        <div class="mb-3">
                            <label for="input_img" class="form-label">Product Image</label>
                            <div class="d-flex">
                                <div>
                                    <input type="file" id="input_img" class="form-control py-1" name="product_image">
                                    @error('product_image')
                                    <span class="font-italic text-danger ">{{ $message }}</span>
                                    @enderror
                                </div>
                                <img id="out_img" src="{{ \App\Http\Helpers\Helper::getPath('products', $product->image) }}" class="ml-2 img-fluid" style="height: 123px">
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control" id="product_maker" name="maker_id">
                                ...
                                <option value="{{ $maker->id }}" {{ old('maker_id', $product->maker_id ?? '' )  == $maker->id ? "selected" : "" }}> {{ $maker->name }} </option>
                                @endforeach
                                @endif
                            </select>
                            @error('maker_id')
                        ...
                        <div class="mb-3">
                            ...
                            @if(isset($categories))
                                ...
                                <option 
                                    @if (old('category')) 
                                        {{ in_array( $category->id, (array) old('category') ) ? "selected" : "" }} 
                                    @else 
                ...
                <div class="form-row">
                    <div class="col">
                        ...
                        <textarea class="form-control" id="example-textarea" rows="10" name="content">{{ old('content', $product->content ?? '') }}</textarea>
                        @error('content')
                        <span class="font-italic text-danger ">{{ $message }}</span>
                        @enderror
                    </div>
